...
extern	C_MainWindow*					pMainWindow;
		...
		C_SettingsWindow*				pSettingsWindow				= 0;
extern	bool							bshown;
...
//////////////////////////////////////////////////////////////////////////
C_MainWindow::C_MainWindow( QWidget *parent /*= 0*/, Qt::WFlags flags /*= 0 */ )
	: QMainWindow(parent, flags)
{
	ui.setupUi(this);

//	ifstream licfile("lc.data");
//	if(licfile.is_open()){
//
//		string lcstr;
//		lcstr.append(plcstr);
...
//		licfile >> lc2str;
//
//		if(lcstr.compare(lc2str) == 0){
//			lc=true;
//		}else{
//			lc=false;
//			plicense = new licenter();
//			plicense->show();
//		}
//
//	}else{
...
//		imwrite("tm_data.png", pCoordinateTransform->getImageRepresentation());
	...
	pSettingsWindow = new C_SettingsWindow();
//	pSettingsWindow->windowFlags(pSettingsWindow->windowFlags() &~(Qt::WindowMinimizeButtonHint));
//	pSettingsWindow->windowFlags(pSettingsWindow->windowFlags() &~(Qt::WindowMaximizeButtonHint));

	pAboutWindow = new C_AboutWindow();
	pCalibrationWindow = new C_CalibrationWindow();

	pSystemTrayMenu = new QMenu();

	pSystemTrayMenuRestore		= pSystemTrayMenu->addAction(QIcon(QString::fromUtf8(":/main/restore.png")), QObject::tr("Восстановить главное окно"));
	pSystemTrayMenu->addSeparator();
	pSystemTrayMenuCalibration	= pSystemTrayMenu->addAction(QIcon(QString::fromUtf8(":/main/calibrate.png")), QObject::tr("Калибровка"));
	pSystemTrayMenuSettings		= pSystemTrayMenu->addAction(QIcon(QString::fromUtf8(":/main/cogwheel.png")), QObject::tr("Настройки"));
	pSystemTrayMenu->addSeparator();
	pSystemTrayMenuAbout		= pSystemTrayMenu->addAction(QIcon(QString::fromUtf8(":/main/help.png")), QObject::tr("О программе..."));
	pSystemTrayMenu->addSeparator();
	pSystemTrayMenuClose		= pSystemTrayMenu->addAction(QIcon(QString::fromUtf8(":/main/close.png")), QObject::tr("Закрыть программу"));

	QIcon oTrayIcon;
	oTrayIcon.addFile(QString::fromUtf8(":/main/webcam.png"), QSize(), QIcon::Normal, QIcon::Off);
	pSystemTray = new QSystemTrayIcon(oTrayIcon);
	pSystemTray->setContextMenu(pSystemTrayMenu);
	pSystemTray->show();
	...
	QObject::connect(	ui.ButtonSettings,					SIGNAL(clicked()),					pSettingsWindow,		SLOT(show())					);
	QObject::connect(	pSystemTrayMenuSettings,			SIGNAL(triggered()),				pSettingsWindow,		SLOT(show())					);

	QObject::connect(	pSystemTrayMenuClose,				SIGNAL(triggered()),				this,					SLOT(close())					);

	QObject::connect(	ui.ButtonToTray,					SIGNAL(clicked()),					this,					SLOT(hide())					);
	QObject::connect(	pSystemTrayMenuRestore,				SIGNAL(triggered()),				this,					SLOT(show())					);
...
C_MainWindow::~C_MainWindow(){
	...
	delete pSystemTray;
	delete pSystemTrayMenu;

	delete pSystemTrayMenuRestore;
	delete pSystemTrayMenuCalibration;
	delete pSystemTrayMenuSettings;
	delete pSystemTrayMenuAbout;
	delete pSystemTrayMenuClose;

	delete pSettingsWindow;
	delete pAboutWindow;
	delete pCalibrationWindow;
...
//////////////////////////////////////////////////////////////////////////
void C_MainWindow::closeEvent( QCloseEvent* pEvent )
{
	if(pVideoCapture != 0){
	...
	if(pSettingsWindow!=0)
		pSettingsWindow->close();
	if(pSystemTray!=0)
		pSystemTray->hide();
}

void C_MainWindow::timerEvent( QTimerEvent* pEvent ){
...
//////////////////////////////////////////////////////////////////////////
void C_MainWindow::Calibration( void )
{
...
//////////////////////////////////////////////////////////////////////////
void C_MainWindow::slotSystemNoCamera()
{
	if(pRawMouse!=0){
	...
	pSystemTray->showMessage(QObject::tr("Интерактивная ручка"), QObject::tr("Идёт поиск датчика..."), QSystemTrayIcon::Information, 1000);
}

//////////////////////////////////////////////////////////////////////////
void C_MainWindow::slotSystemAllOk()
{
	if(pRawMouse!=0){
	...
	pSystemTray->showMessage(QObject::tr("Интерактивная ручка"), QObject::tr("Система в рабочем состоянии"), QSystemTrayIcon::Information, 1000);
}

//////////////////////////////////////////////////////////////////////////
void C_MainWindow::slotSystemNotCalibrated()
{
	if(findCaptureTimer!=0){
		killTimer(findCaptureTimer);
		findCaptureTimer=0;
	...
	pSystemTray->showMessage(QObject::tr("Интерактивная ручка"), QObject::tr("Необходима калибровка"), QSystemTrayIcon::Information, 1000);
}
...
//////////////////////////////////////////////////////////////////////////
void C_MainWindow::slotPulledOut(){
